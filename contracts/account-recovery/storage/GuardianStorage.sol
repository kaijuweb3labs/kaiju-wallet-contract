...
contract GuardianStorage is IGuardianStorage {
    ...
    modifier onlyModule(address _wallet) {
        ...
        require(
            Safe(payable(_wallet)).isModuleEnabled(msg.sender),
            "GS: methold only callable by an enabled module"
        );
        _;
    }
    ...
    function addGuardian(
        ...
        require(
            _guardian != address(0) &&
                _guardian != SENTINEL_GUARDIANS &&
                _guardian != _wallet,
            "GS: invalid guardian"
        );
        require(
            !Safe(payable(_wallet)).isOwner(_guardian),
            "GS: guardian cannot be an owner"
        );
        GuardianStorageEntry storage entry = entries[_wallet];
        require(
            entry.guardians[_guardian] == address(0),
            "GS: duplicate guardian"
        );
        if (entry.count == 0) {
    ...
    function revokeGuardian(
        ...
        GuardianStorageEntry storage entry = entries[_wallet];
        require(
            _guardian != address(0) && _guardian != SENTINEL_GUARDIANS,
            "GS: invalid guardian"
        );
        require(
            entry.guardians[_prevGuardian] == _guardian,
            "GS: invalid previous guardian"
        );
    ...
    function isGuardian(
        ...
        return
            _guardian != SENTINEL_GUARDIANS &&
            entries[_wallet].guardians[_guardian] != address(0);
    }
